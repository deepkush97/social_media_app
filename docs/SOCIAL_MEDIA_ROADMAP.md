...
- [x] **Emit `POST_CREATED` consumed** — `SocialSubscriberService.onPostCreated()` receives `tags` in payload and calls `boostTagWeight()` (creates `INTERESTED_IN` from author to tags, weight 1.0) and `trackPostCreation()` (creates `CREATED` + `TAGGED` edges). Tags are extracted once at creation time by the posts service and stored on the entity.
- [x] **Consume `USER_CREATED` in social subscriber** — `USER_CREATED` is emitted by the gateway auth service on signup but had **no consumer**. Added `SocialSubscriberService.onUserCreated()` → `SocialService.ensureUserNode(userId)` which `MERGE`s a `:User {id}` node proactively at signup. Previously User nodes were created lazily via MERGE in other operations.
- [x] **`POST_CREATED` tag weight increment lowered to 0.5** — `boostTagWeight(userId, tags, 1.0)` in the `POST_CREATED` handler changed to 0.5 to prevent creator-side accumulation from outpacing consumer-side interest (like-based boost stays at 1.0). This reduces the spam surface while still rewarding genuine tag affinity.
